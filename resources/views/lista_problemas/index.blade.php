...
function exportF(elem) {
  ...
  elem.setAttribute("download", "lista_problemas.xls"); // Choose the file name
  return false;
}
</script>
...
<div class="container">
      <div class="row justify-content-left">
        <div class="col-md-3">
            ...
            <a href="{{url('/lista_problemas/create')}}" class="btn btn-success" >Agregar Nuevo Problema</a>
        ...
        <div class="col-md-12">
            ...
            <div style="overflow-x:auto;">      
             <table class="table table-light table-striped table-bordered" id="example"  style="width:100%">
                ...
                <tbody>
                 @foreach($lista_problemas as $producto)
                  <tr>
                    <td>{{$loop->iteration}}</td>
                    <td>{{$producto->id_maestra}}</td>
                    <td>{{$producto->nom_problema}}</td>
                    <td>
                        <div class="btn-group">
                         <form class="btn-group" method="post" action="{{url('lista_problemas/'.$producto->id)}}">
                            <a href="{{url('/lista_problemas/'.$producto->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-pencil" ></i></a>
                            {{csrf_field()}}
                            {{method_field('DELETE')}}
