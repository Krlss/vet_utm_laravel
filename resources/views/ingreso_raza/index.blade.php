...
function exportF(elem) {
  ...
  elem.setAttribute("download", "Razas.xls"); // Choose the file name
  return false;
}
</script>
...
<div class="container">
      <div class="row justify-content-left">
        <div class="col-md-3">
			...
			<a href="{{url('/ingreso_raza/create')}}" class="btn btn-success" >Agregar nueva Raza</a>
        ...
        <div class="col-md-12">
		    ...
		    <div style="overflow-x:auto;">      
		     <table class="table table-light table-striped table-bordered" id="example"  style="width:100%">
			    ...
			    <tbody>
				 @foreach($raza as $producto)
				  <tr>
					 <td>{{$loop->iteration}}</td>
					 <td>{{$producto->nom_raza}}</td>
				     <td>{{$producto->especie}}</td>
						
					<td>
					 	<div class="btn-group">
					 	 <form class="btn-group" method="post" action="{{url('ingreso_raza/'.$producto->id)}}">
					 		<a href="{{url('/ingreso_raza/'.$producto->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-pencil" ></i></a>
					 		{{csrf_field()}}
					 		{{method_field('DELETE')}}
