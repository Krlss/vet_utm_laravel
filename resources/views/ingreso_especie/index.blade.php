...
function exportF(elem) {
  ...
  elem.setAttribute("download", "Especies.xls"); // Choose the file name
  return false;
}
</script>
...
<div class="container">
      <div class="row justify-content-left">
        <div class="col-md-3">
			...
			<a href="{{url('/ingreso_especie/create')}}" class="btn btn-success" >Agregar nueva Especie</a>
				 ...
				 @foreach($especie as $producto)
				  <tr>
				   ...
				   <td>{{$producto->especie}}</td>
					...
					<td>
					 	<div class="btn-group">
					 	 <form class="btn-group" method="post" action="{{url('ingreso_especie/'.$producto->id)}}">
					 		<a href="{{url('/ingreso_especie/'.$producto->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-pencil" ></i></a>
					 		{{csrf_field()}}
					 		{{method_field('DELETE')}}
