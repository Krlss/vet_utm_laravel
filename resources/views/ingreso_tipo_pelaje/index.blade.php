...
function exportF(elem) {
  ...
  elem.setAttribute("download", "Tipo_pelaje.xls"); // Choose the file name
  return false;
}
</script>
...
<div class="container">
      <div class="row justify-content-left">
        <div class="col-md-3">
			...
			<a href="{{url('/ingreso_tipo_pelaje/create')}}" class="btn btn-success" >Agregar nuevo Parámetro</a>
        ...
        <div class="col-md-12">
		    ...
		    <div style="overflow-x:auto;">      
		     <table class="table table-light table-striped table-bordered" id="example"  style="width:100%">
			    ...
			    <tbody>
				 @foreach($pelaje as $producto)
				  <tr>
					 <td>{{$loop->iteration}}</td>
				   <td>{{$producto->especie}}</td>
				   <td>{{$producto->tipo_pelaje}}</td>
						
					<td>
					 	<div class="btn-group">
		            	<a href="{{url('/ingreso_tipo_pelaje/'.$producto->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-pencil" ></i></a>
					  		

					 	 <form method="post" action="{{url('ingreso_tipo_pelaje/'.$producto->id)}}">
					 		{{csrf_field()}}
					 		{{method_field('DELETE')}}
