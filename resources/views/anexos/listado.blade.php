...
@foreach($anexo as $pet)
  <tbody>
    
    <tr>
      
      <td>{{$pet->cod_mascota}}</td>
      <td>{{$pet->descripcion_anexo}}</td> 
      <td>{{$pet->fecha_anexo}}</td>
      <td>{{$pet->documento_anexo}}</td> 

            <td>
                <div  class="btn-group">
                    <form  method="post" action="{{url('/anexos/'.$pet->id)}}">
                        <a title="Ver Anexos" href="{{url($pet->documento_anexo)}}" class="btn btn-warning" target ="_blank"><i class="fa fa-book" ></i></a>
