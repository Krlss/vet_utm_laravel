...
function exportF(elem) {
  ...
  elem.setAttribute("download", "desparasitantes.xls"); // Choose the file name
  return false;
}
</script>
...
<div class="container">
      <div class="row justify-content-left">
        <div class="col-md-3">
			...
			<a href="{{url('/desparasitante/create')}}" class="btn btn-success" >Agregar nuevo Desparasitante</a>
		     ...
		     <table class="table table-light table-striped table-bordered" id="example"  style="width:100%">
			    <thead class="thead-light">
				  <tr>
				    <th>#</th>
				    <th>Especie</th>		
				   	<th>Nom. Desparasitante</th>
                    <th>Acciones</th>	
				  </tr>

			    </thead>
			    <tbody>
				 @foreach($desparasitante as $producto)
				  <tr>
					 <td>{{$loop->iteration}}</td>
				   <td>{{$producto->especie}}</td>
				   <td>{{$producto->nom_desparasitante}}</td>
						
					<td>
					 	<div class="btn-group">
		            	 
					  	 	
					 	 <form class="btn-group" method="post" action="{{url('desparasitante/'.$producto->id)}}">
					 	 	<a href="{{url('/desparasitante/'.$producto->id.'/edit')}}" class="btn btn-warning"><i class="fa fa-pencil" aria-hidden="true" ></i></a>
					 		{{csrf_field()}}
					 		{{method_field('DELETE')}}
